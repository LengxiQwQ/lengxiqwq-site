...
### 1. 私有内容仓接管文章
- [ ] 在 `lengxiqwq-site-content/content/posts/` 建立正式文章目录。
- [ ] 即使暂时没有文章，也保留空目录占位方式，确保构建时不会继续使用公开仓 Firefly 示例文章。
- [ ] 验证部署后首页、归档、分类、标签、搜索、RSS 中不再出现模板演示文章。

**建议：**
暂时没准备好正式文章时，宁可让文章列表为空，也不要保留模板演示文章。

### 2. 修正 `sync-content.ts` 的清理范围
- [ ] 当前 `cleanableCollections` 只有 `projects`、`posts`，增加 `dynamic`。
- [ ] 在私有仓补齐 `content/spec/guestbook.md` 后，再考虑让 `spec` 也采用“私有仓完全接管”策略。
- [ ] 验证私有仓内容同步后不会和公开仓预设内容混合。
...
### 4. 动态 / 说说
当前仍有 4 条 Firefly 演示动态：
- [ ] “今天天气真不错，流萤真可爱。”
- [ ] “这是一条测试动态！”
- [ ] “又是美好的一天！”
- [ ] “飞萤之火自无梦的长夜亮起，绽放在终竟的明天。”
- [ ] 在私有仓创建 `content/dynamic/`，改成自己的动态。
...
需要：
- [ ] 删除模板默认友链。
- [ ] 添加真实朋友 / 项目 / 常用站点，或暂时留空。
- [ ] 保留已经完成的“申请友链”页面和本站信息卡。
...
### 9. 音乐播放器
当前歌曲和封面仍为 Firefly 原仓内容：
- [ ] 《使一颗心免于哀伤》
- [ ] 原模板 MP3
- [ ] 原模板封面

需要：
- [ ] 换成自己的音乐列表，或关闭本地音乐播放器。
- [ ] 决定继续 local 模式还是改为 Meting/其他数据源。
